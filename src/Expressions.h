...
class VariableExpression : public Expression {
 ...
 public:
  ...
  virtual int GetType(void) const { return Exp_Variable; }

  virtual void GenerateCode(std::ostream& out, int) const { out << m_name; }

  const std::string& GetVariableName(void) const { return m_name; }
};

// *****************************************************************************************
class LocalVariableExpression : public VariableExpression {
 public:
  explicit LocalVariableExpression(const std::string& name) : VariableExpression(name) {}

  virtual int GetType(void) const { return Exp_LocalVariable; }
};
...
class ConstantExpression : public Expression {
 protected:
  ...
  ConstantExpression() { m_isLiteral = false; }

 public:
  explicit ConstantExpression(const std::string& str) { set(str); }

  explicit ConstantExpression(unsigned int value) { set(value); }

  explicit ConstantExpression(float value) { set(value); }
  ...
  void set(float value) {
    ...
    if (builder.str().find('.') == std::string::npos) builder << ".0";

    m_isLiteral = false;
    m_text = builder.str();
  }

  virtual int GetType(void) const { return Exp_Constant; }

  virtual void GenerateCode(std::ostream& out, int) const { out << m_text; }

  bool IsLiteral(void) const { return m_isLiteral; }

  bool IsLabel(void) const {
    if (!m_isLiteral || m_text.size() < 3) return false;

    for (int i = 1; i < (int)(m_text.size() - 1); ++i) {
      char c = m_text[i];

      bool isValidChar =
          (i > 1 && c >= '0' && c <= '9') || (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') || c == '_';
...
class LiteralConstantExpression : public ConstantExpression {
 public:
  explicit LiteralConstantExpression(const char* name) { m_text = name; }
};
...
class RootTableExpression : public Expression {
 public:
  ...
  virtual int GetType(void) const { return Exp_RootTable; }

  virtual void GenerateCode(std::ostream& out, int) const { out << "getroottable()"; }
};
...
class NullExpression : public Expression {
 public:
  ...
  virtual int GetType(void) const { return Exp_Null; }

  virtual void GenerateCode(std::ostream& out, int) const { out << "null"; }
};
...
class OperatorExpression : public Expression {
 ...
 public:
  virtual int GetType(void) const { return Exp_Operator; }

  int GetOperatorType(void) const { return m_operator; }
...
class UnaryOperatorExpression : public OperatorExpression {
 ...
 public:
  ...
  virtual void GenerateCode(std::ostream& out, int n) const {
    ...
    if (OPER_SPECIAL_MARKER == (m_operator & OPER_MASK)) out << ' ';

    bool parenthesis = m_arg->IsOperator() && (static_pointer_cast<OperatorExpression>(m_arg)->GetOperatorPriority() <
                                               this->GetOperatorPriority());
    GenerateArgument(out, n, m_arg, parenthesis);
  }

  virtual int GetOperatorPriority(void) const { return 200; }
};
...
class UnaryPostfixOperatorExpression : public OperatorExpression {
 ...
 public:
  ...
  virtual int GetOperatorPriority(void) const { return 300; }
};
 ...
 public:
  ...
  virtual void GenerateCode(std::ostream& out, int n) const {
    ...
    if (m_arg1->IsOperator()) {
      if (!rightToLeft) {
        if (static_pointer_cast<OperatorExpression>(m_arg1)->GetOperatorPriority() < myPriority) leftParenthesis = true;
      } else {
...
class ConditionOperatorExpression : public OperatorExpression {
 ...
 public:
  ...
  virtual void GenerateCode(std::ostream& out, int n) const {
    int myPriority = GetOperatorPriority();
    bool condParethesis = (m_condition->IsOperator() &&
                           (static_pointer_cast<OperatorExpression>(m_condition)->GetOperatorPriority() <= myPriority));
    bool arg1Parenthesis = (m_whenTrue->IsOperator() &&
                            (static_pointer_cast<OperatorExpression>(m_whenTrue)->GetOperatorPriority() <= myPriority));
    bool arg2Parenthesis = (m_whenFalse->IsOperator() &&
                            (static_pointer_cast<OperatorExpression>(m_whenFalse)->GetOperatorPriority() < myPriority));
  ...
  virtual int GetOperatorPriority(void) const { return 60; }
...
class DelegateOperatorExpression : public OperatorExpression {
 ...
 public:
  ...
  virtual void GenerateCode(std::ostream& out, int n) const {
    ...
    int myPriority = GetOperatorPriority();
    bool arg1Parenthesis = (m_arg1->IsOperator() &&
                            (static_pointer_cast<OperatorExpression>(m_arg1)->GetOperatorPriority() <= myPriority));
    bool arg2Parenthesis = (m_arg2->IsOperator() &&
                            (static_pointer_cast<OperatorExpression>(m_arg2)->GetOperatorPriority() <= myPriority));
...
class ArrayIndexingExpression : public OperatorExpression {
 ...
 public:
  ...
  bool IsSimpleMemberDeref(void) const {
    if (!ConstantExpression::AsLabelExpression(m_indexer)) return false;

    if (m_obj->GetType() == Exp_Operator &&
        static_pointer_cast<OperatorExpression>(m_obj)->GetOperatorType() == OPER_ARRAYIND) {
      ...
      return static_pointer_cast<VariableExpression>(m_obj)->GetVariableName() ==
             "this";  // TODO: Check for local variable deref
                      // return true;
    } else {
      return false;
    }
  ...
  void GenerateCode(std::ostream& out, int n, const char* labelsDelimiter, bool allowExplicitThis) const {
    bool parenthesis = m_obj->IsOperator() && static_pointer_cast<OperatorExpression>(m_obj)->GetOperatorPriority() <
                                                  this->GetOperatorPriority();
    ...
    if (labelIndexer) {
      ...
      if (m_obj->GetType() == Exp_RootTable) {
        out << "::";
      } else if (m_obj->GetType() == Exp_Operator &&
                 static_pointer_cast<OperatorExpression>(m_obj)->GetOperatorType() == OPER_ARRAYIND) {
  ...
  virtual void GenerateCode(std::ostream& out, int n) const { GenerateCode(out, n, ".", true); }
...
class FunctionCallExpression : public Expression {
 ...
 public:
  explicit FunctionCallExpression(ExpressionPtr func) { m_function = func; }

  void AddArgument(ExpressionPtr arg) { m_arguments.push_back(arg); }

  virtual int GetType(void) const { return Exp_FunctionCall; }
...
class FunctionExpression : public Expression {
 ...
 public:
  explicit FunctionExpression(int functionIndex) : m_FunctionIndex(functionIndex) {}

  void SetName(const std::string& name) { m_name = name; }

  virtual int GetType(void) const { return Exp_Function; }
};
...
class NewTableExpression : public TableBaseExpression {
 ...
 public:
  ...
  virtual int GetType(void) const { return Exp_NewTableExpression; }
  ...
  virtual void GenerateCode(std::ostream& out, int n) const {
    ...
    out << "{" << std::endl;
    for (vector<std::pair<ExpressionPtr, ExpressionPtr> >::const_iterator i = m_Elements.begin(); i != m_Elements.end();
         ++i) {
      out << ::indent(n + 1);
...
class NewArrayExpression : public Expression {
 ...
 public:
  ...
  virtual int GetType(void) const { return Exp_NewArrayExpression; }
  ...
  void AddElement(ExpressionPtr value) { m_Elements.push_back(value); }
};
...
class NewClassExpression : public TableBaseExpression {
 ...
 public:
  ...
  virtual int GetType(void) const { return Exp_NewClassExpression; }

  void GenerateCode(std::ostream& out, int n) const {
    out << "class ";

    if (!m_Name.empty()) out << m_Name << ' ';
...
// ******************************************************************************************************************************************************************************
inline void TableBaseExpression::GenerateElementCode(ExpressionPtr key, ExpressionPtr value, char eolChar,
                                                     std::ostream& out, int n) const {
  if (value->GetType() == Exp_Function || value->GetType() == Exp_NewClassExpression) {
    ...
    } else if (key->GetType() == Exp_Operator &&
               static_pointer_cast<OperatorExpression>(key)->GetOperatorType() == OperatorExpression::OPER_ARRAYIND) {
