...
const NutFunction* NutFunction::FindFunction(const std::string& name) const {
  ...
  if (localName.empty()) return NULL;
  ...
  if (pos < 0 || pos >= (int)m_Functions.size()) return NULL;
...
const NutFunction& NutFunction::GetFunction(int i) const {
  if (i < 0 || i >= (int)m_Functions.size()) throw Error("Invalid index in GetFunction.");
...
void NutFunction::Load(BinaryReader& reader) {
  ...
  for (int i = 0; i < nLiterals; ++i) m_Literals[i].Load(reader);

  reader.ConfirmOnPart();

  m_Parameters.resize((size_t)nParameters);
  for (int i = 0; i < nParameters; ++i) reader.ReadSQStringObject(m_Parameters[i]);
...
void NutScript::LoadFromFile(const char* fileName) {
  ...
  if (file.fail()) throw Error("Unable to open file: \"%s\"", fileName);
...
void NutScript::LoadFromStream(std::istream& in) {
  ...
  // Magic
  if (reader.ReadUInt16() != 0xFAFA) throw BadFormatError();

  if (reader.CheckArch() == AT_UNKNOWN) throw Error("NUT file compiled for different architecture that expected.");

  if (reader.ReadArchInt() != 'SQIR') throw BadFormatError();

  if (reader.CheckCharSize() == CS_UNKNOWN) throw Error("NUT file compiled for different size of char that expected.");

  m_main.Load(reader);

  if (reader.ReadArchInt() != 'TAIL') throw BadFormatError();
}
...
bool Eq(const NutFunction::Instruction& a, const NutFunction::Instruction& b) {
  ...
  {
    return a.arg0 == b.arg0 && (std::abs(a.arg1_float - b.arg1_float) < 0.00001) && a.arg2 == b.arg2 &&
           a.arg3 == b.arg3;
  } else {
...
bool NutFunction::DoCompare(const NutFunction& other, const std::string parentName, std::ostream& out) const {
  ...
  if (!parentName.empty()) nameFormatter << parentName << "::";
  ...
  if (m_Functions.size() != other.m_Functions.size()) {
    out << name << ':' << std::endl;
    out << "    - different number of subfunctions: " << m_Functions.size() << " to " << other.m_Functions.size()
        << std::endl;
    functionsOk = false;
  } else {
    for (size_t i = 0; i < m_Functions.size(); ++i)
      if (!m_Functions[i].DoCompare(other.m_Functions[i], name, out)) functionsOk = false;

    out << name << ':' << std::endl;
  }

  if (m_Literals.size() != other.m_Literals.size()) {
    out << "    - different number of literals: " << m_Literals.size() << " to " << other.m_Literals.size()
        << std::endl;
    literalsOk = false;
  } else {
    for (size_t i = 0; i < m_Literals.size(); ++i)
      if (m_Literals[i] != other.m_Literals[i]) {
        out << "    - different literals @ " << i << ": \"" << m_Literals[i] << "\" and \"" << other.m_Literals[i]
            << "\"" << std::endl;
        literalsOk = false;
      }
  }
  ...
  if (m_Parameters.size() != other.m_Parameters.size()) {
    out << "    - different number of parameters: " << m_Parameters.size() << " to " << other.m_Parameters.size()
        << std::endl;
    parametersOk = false;
  }

  if (m_OuterValues.size() != other.m_OuterValues.size()) {
    out << "    - different number of outer values: " << m_OuterValues.size() << " to " << other.m_OuterValues.size()
        << std::endl;
    outerValuesOk = false;
  } else {
    for (size_t i = 0; i < m_OuterValues.size(); ++i)
      if (m_OuterValues[i].src != other.m_OuterValues[i].src) {
        out << "    - different outer value source @ " << i << ": " << m_OuterValues[i].src << " and "
            << other.m_OuterValues[i].src << std::endl;
        outerValuesOk = false;
      }
  }

  if (m_Instructions.size() != other.m_Instructions.size()) {
    out << "    - different number of instructions: " << m_Instructions.size() << " to " << other.m_Instructions.size()
        << std::endl;
