...
class BinaryReader {
 ...
 public:
  ...
  void Read(void* buffer, uint64_t size) {
    ...
    if (_in.fail()) throw Error("I/O Error while reading from file.");
  }
  ...
  // ******************************************************************************
  uint64_t ReadArchUInt(void) { return targetArch == AT_X32 ? ReadUInt32() : ReadUInt64(); }

  // ******************************************************************************
  int64_t ReadArchInt(void) { return targetArch == AT_X32 ? ReadInt32() : ReadInt64(); }

  // ******************************************************************************
  void ConfirmOnPart(void) {
    int64_t data = ReadArchInt();

    if (data != 'PART') throw Error("Bad format of source binary file (PART marker was not match).");
  }
  ...
  void ReadSQString(std::string& str) {
    ...
    if (buffer.size() < bytesLen + nullTerminatorSize) buffer.resize(bytesLen + nullTerminatorSize);
  ...
  std::streampos position() { return _in.tellg(); }
};
