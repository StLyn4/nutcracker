...
class Statement : public enable_shared_from_this<Statement> {
 public:
  ...
  virtual shared_ptr<Statement> Postprocess(void) { return shared_from_this(); }
...
class EmptyStatement : public Statement {
 public:
  virtual int GetType(void) const { return Stat_Empty; }
  ...
  static StatementPtr Get(void) {
    ...
    if (!s_instance) s_instance = StatementPtr(new EmptyStatement);
...
class ExpressionStatement : public Statement {
 ...
 public:
  explicit ExpressionStatement(ExpressionPtr exp) { m_Expression = exp; }

  virtual int GetType(void) const { return Stat_Expression; }
  ...
  virtual StatementPtr Postprocess(void) {
    if (!m_Expression) return EmptyStatement::Get();

    return Statement::Postprocess();
  }

  void Clear(void) { m_Expression = shared_ptr<Expression>(); }

  bool Equals(ExpressionPtr right) const { return m_Expression == right; }

  const ExpressionPtr GetExpression(void) const { return m_Expression; }
};
...
class BlockStatement : public Statement {
 ...
 public:
  ...
  void Add(StatementPtr statement) { m_Statements.push_back(statement); }

  std::vector<StatementPtr>& Statements(void) { return m_Statements; }

  const std::vector<StatementPtr>& Statements(void) const { return m_Statements; }

  virtual int GetType(void) const { return Stat_Block; }
  ...
  void GenerateBlockContentCode(std::ostream& out, int n) const {
    ...
    for (vector<StatementPtr>::const_iterator i = m_Statements.begin(); i != m_Statements.end(); ++i) {
      ...
      if (prevStatement && (separatedStatement || pendingSpace)) out << std::endl;
...
class IfStatement : public Statement {
 ...
 public:
  ...
  virtual int GetType(void) const { return Stat_If; }
  ...
  const ExpressionPtr GetConditionExpression(void) const { return m_Condition; }
  ...
  virtual StatementPtr Postprocess(void) {
    ...
    if (m_WhenFalse) m_WhenFalse = m_WhenFalse->Postprocess();

    if (m_Canceled) return EmptyStatement::Get();
...
class LocalVarInitStatement : public Statement {
 ...
 public:
  explicit LocalVarInitStatement(const std::string& name, int64_t stackAddress, int64_t startAddress,
                                 int64_t endAddress, ExpressionPtr init = ExpressionPtr()) {
    m_Name = name;
  ...
  virtual int GetType(void) const { return Stat_LocalVar; }
  ...
  const std::string& GetVarName(void) const { return m_Name; }
...
class ReturnStatement : public Statement {
 ...
 public:
  ...
  explicit ReturnStatement(ExpressionPtr retExpr) { m_Expression = retExpr; }

  virtual int GetType(void) const { return Stat_Return; }
...
class ThrowStatement : public Statement {
 ...
 public:
  explicit ThrowStatement(ExpressionPtr retExpr) { m_Expression = retExpr; }

  virtual int GetType(void) const { return Stat_Throw; }
...
class YieldStatement : public Statement {
 ...
 public:
  explicit YieldStatement(ExpressionPtr retExpr) { m_Expression = retExpr; }

  virtual int GetType(void) const { return Stat_Yield; }
...
class TryCatchStatement : public Statement {
 ...
 public:
  ...
  virtual int GetType(void) const { return Stat_TryCatch; }
...
class BreakStatement : public Statement {
 public:
  virtual int GetType(void) const { return Stat_Break; }

  virtual void GenerateCode(std::ostream& out, int n) const { out << ::indent(n) << "break;" << std::endl; }
};

// *******************************************************************************************
class ContinueStatement : public Statement {
 public:
  virtual int GetType(void) const { return Stat_Continue; }

  virtual void GenerateCode(std::ostream& out, int n) const { out << ::indent(n) << "continue;" << std::endl; }
};
...
class CommentStatement : public Statement {
 ...
 public:
  explicit CommentStatement(const std::string& text) { m_Text = text; }

  virtual int GetType(void) const { return Stat_Comment; }

  virtual void GenerateCode(std::ostream& out, int n) const { out << ::indent(n) << " // " << m_Text << std::endl; }
};
...
class LoopBaseStatement : public Statement {
 ...
 public:
  void SetLoopBlock(const BlockState& blockState) {
    ...
    if (blockState.inLoop) m_LoopEndAddress += 1;  // Last JMP of loop
  }
...
class ForStatement : public LoopBaseStatement {
 ...
 public:
  explicit ForStatement(StatementPtr initialization, ExpressionPtr condition, StatementPtr incrementation,
                        StatementPtr block) {
  ...
  virtual int GetType(void) const { return Stat_For; }
  ...
  virtual void GenerateCode(std::ostream& out, int n) const {
    ...
    if (m_Initialization) GenerateStatementInline(out, n, m_Initialization);

    out << "; ";

    if (m_Condition) m_Condition->GenerateCode(out, n);

    out << "; ";

    if (m_Incrementation) GenerateStatementInline(out, n, m_Incrementation);
...
class WhileStatement : public LoopBaseStatement {
 ...
 public:
  ...
  virtual int GetType(void) const { return Stat_While; }
  ...
  StatementPtr GetForIncrementStatement(void) {
    if (m_Block->GetType() != Stat_Block) return StatementPtr();
    ...
    const ExpressionPtr lastExpresion =
        static_pointer_cast<ExpressionStatement>(block->Statements().back())->GetExpression();
    if (!lastExpresion || !lastExpresion->IsOperator()) return StatementPtr();

    int opType = static_pointer_cast<OperatorExpression>(lastExpresion)->GetOperatorType();
    if (opType != '++' && opType != '--' && opType != '+=' && opType != '-=' && opType != '=') return StatementPtr();
  ...
  StatementPtr TryGenerateForStatement(StatementPtr prevStatement) {
    if (m_LoopFlags & BlockState::UsedBackwardJumpContinue) return StatementPtr();
    ...
    if ((prevStatement->GetType() != Stat_LocalVar) && !incrementStatement) return StatementPtr();

    LoopBaseStatementPtr forStatement =
        LoopBaseStatementPtr(new ForStatement(prevStatement, m_Condition, incrementStatement, m_Block));
...
class DoWhileStatement : public LoopBaseStatement {
 ...
 public:
  ...
  virtual int GetType(void) const { return Stat_DoWhile; }
...
class ForeachStatement : public LoopBaseStatement {
 ...
 public:
  ...
  virtual int GetType(void) const { return Stat_Foreach; }

  virtual void GenerateCode(std::ostream& out, int n) const {
    out << ::indent(n) << "foreach (";

    if (m_Key &&
        (!m_Key->IsVariable() || static_pointer_cast<VariableExpression>(m_Key)->GetVariableName() != "@INDEX@"))
...
class SwitchStatement : public Statement {
 ...
 public:
  ...
  virtual int GetType(void) const { return Stat_Switch; }
...
class CaseStatement : public Statement {
 ...
 public:
  explicit CaseStatement(ExpressionPtr value) { m_Value = value; }

  virtual int GetType(void) const { return Stat_Case; }
