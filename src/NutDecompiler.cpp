...
enum BitWiseOpcode { BW_AND = 0, BW_OR = 2, BW_XOR = 3, BW_SHIFTL = 4, BW_SHIFTR = 5, BW_USHIFTR = 6 };

enum ComparisionOpcode { CMP_G = 0, CMP_GE = 2, CMP_L = 3, CMP_LE = 4 };

const char* OpcodeNames[] = {"OP_LINE",        "OP_LOAD",          "OP_LOADINT",    "OP_LOADFLOAT",
                             "OP_DLOAD",       "OP_TAILCALL",      "OP_CALL",       "OP_PREPCALL",
                             "OP_PREPCALLK",   "OP_GETK",          "OP_MOVE",       "OP_NEWSLOT",
                             "OP_DELETE",      "OP_SET",           "OP_GET",        "OP_EQ",
                             "OP_NE",          "OP_ARITH",         "OP_BITW",       "OP_RETURN",
                             "OP_LOADNULLS",   "OP_LOADROOTTABLE", "OP_LOADBOOL",   "OP_DMOVE",
                             "OP_JMP",         "OP_JNZ",           "OP_JZ",         "OP_LOADFREEVAR",
                             "OP_VARGC",       "OP_GETVARGV",      "OP_NEWTABLE",   "OP_NEWARRAY",
                             "OP_APPENDARRAY", "OP_GETPARENT",     "OP_COMPARITH",  "OP_COMPARITHL",
                             "OP_INC",         "OP_INCL",          "OP_PINC",       "OP_PINCL",
                             "OP_CMP",         "OP_EXISTS",        "OP_INSTANCEOF", "OP_AND",
                             "OP_OR",          "OP_NEG",           "OP_NOT",        "OP_BWNOT",
                             "OP_CLOSURE",     "OP_YIELD",         "OP_RESUME",     "OP_FOREACH",
                             "OP_POSTFOREACH", "OP_DELEGATE",      "OP_CLONE",      "OP_TYPEOF",
                             "OP_PUSHTRAP",    "OP_POPTRAP",       "OP_THROW",      "OP_CLASS",
                             "OP_NEWSLOTA"};
...
class VMState {
 ...
 public:
  ...
  VMState(const NutFunction& parent, int64_t stackSize) : m_Parent(parent) {
    ...
    for (int i = 0; i < (int)parent.m_Instructions.size(); ++i)
      if (parent.m_Instructions[i].op == OP_JNZ && parent.m_Instructions[i].arg1 < 0) m_JNZInstructions.push_back(i);
  }
  ...
  void NextInstruction(void) {
    ...
    for (vector<NutFunction::LocalVarInfo>::const_iterator i = m_Parent.m_Locals.begin(); i != m_Parent.m_Locals.end();
         ++i) {
      if (i->end_op == (m_IP - 1)) {
  ...
  bool EndOfInstructions(void) const { return m_IP >= (int)m_Parent.m_Instructions.size(); }
  ...
  void PushStatement(StatementPtr statement) { m_Block->Add(statement); }
  ...
  void ClearPendingStatement(StatementPtr stat, ExpressionPtr usedExpression) {
    if (stat->GetType() == Stat_Expression) {
      ...
      } else if (usedExpression->GetType() == Exp_Operator &&
                 static_pointer_cast<OperatorExpression>(usedExpression)->GetOperatorType() == '?:') {
        shared_ptr<ConditionOperatorExpression> conditionExp =
            static_pointer_cast<ConditionOperatorExpression>(usedExpression);
  ...
  ExpressionPtr GetVar(int pos) {
    if (pos < 0 || pos >= (int)m_Stack.size()) throw Error("Accessing non valid stack variable.");
    ...
    if (!m_Stack[pos].expression) {
      ...
      for (vector<StatementPtr>::iterator i = m_Stack[pos].pendingStatements.begin();
           i != m_Stack[pos].pendingStatements.end(); ++i)
  ...
  bool InitVar(int pos, ExpressionPtr exp = ExpressionPtr(), bool foreachInit = false) {
    if (pos < 0 || pos >= (int)m_Stack.size()) throw Error("Accessing non valid stack position.");

    if (exp && exp->GetType() == Exp_Null) exp = ExpressionPtr();
    ...
    for (vector<NutFunction::LocalVarInfo>::const_iterator i = m_Parent.m_Locals.begin(); i != m_Parent.m_Locals.end();
      ...
      if (m_Stack[pos].expression && m_Stack[pos].expression->GetType() == Exp_LocalVariable &&
          ...
          !(exp->GetType() == Exp_Operator &&
            static_pointer_cast<OperatorExpression>(exp)->GetOperatorType() == '?:')) {
  ...
  void SetVar(int pos, ExpressionPtr exp, bool expIsStatementLike = false) {
    ...
    if (InitVar(pos, exp)) return;

    if (exp->IsOperator()) expIsStatementLike = true;
  ...
  ExpressionPtr& AtStack(size_t pos) {
    if (pos < 0 || pos >= m_Stack.size()) throw Error("Accessing non valid stack position.");
  ...
  void SwapStacks(StackCopyPtr copy) { m_Stack.swap(*copy); }

  void MergeStackVariable(ExpressionPtr branchCondition_TrueToUseCopy, int pos, StackElement& element,
                          StatementPtr AdditionalPedningStatement) {
    if (m_Stack[pos].expression != element.expression && element.expression) {
      ...
      if (!m_Stack[pos].expression) {
        ...
        ExpressionPtr mergedVar = ExpressionPtr(new ConditionOperatorExpression(
            branchCondition_TrueToUseCopy, element.expression, m_Stack[pos].expression));

        // Prepare merged pending expressions list
        std::vector<StatementPtr> pendingStatements(m_Stack[pos].pendingStatements.begin(),
                                                    m_Stack[pos].pendingStatements.end());
        pendingStatements.insert(pendingStatements.end(), element.pendingStatements.begin(),
                                 element.pendingStatements.end());
        if (AdditionalPedningStatement) pendingStatements.push_back(AdditionalPedningStatement);
  ...
  int IP(void) const { return m_IP; }

  const NutFunction& Parent(void) const { return m_Parent; }
...
class FunctionGeneratingExpression : public FunctionExpression {
 ...
 public:
  explicit FunctionGeneratingExpression(int functionIndex, const NutFunction& function)
      : FunctionExpression(functionIndex), m_Function(function) {}

  void AddDefault(ExpressionPtr value) { m_Defaults.push_back(value); }
...
void NutFunction::DecompileStatement(VMState& state) const {
  ...
  switch (code) {
    ...
    case OP_TAILCALL:
    case OP_CALL: {
      shared_ptr<FunctionCallExpression> exp =
          shared_ptr<FunctionCallExpression>(new FunctionCallExpression(state.GetVar(arg1)));
      for (int i = 1; i < arg3; ++i) exp->AddArgument(state.GetVar(arg2 + i));
    ...
    case OP_GETK:
      state.SetVar(arg0, ExpressionPtr(new ArrayIndexingExpression(
                             state.GetVar(arg2), ExpressionPtr(new ConstantExpression(m_Literals[arg1])))));
      break;
    ...
    case OP_NE: {
      ...
      ExpressionPtr op =
          ExpressionPtr(new BinaryOperatorExpression((code == OP_NE) ? '!=' : '==', state.GetVar(arg2), right));
    ...
    case OP_BITW:
      state.SetVar(arg0, ExpressionPtr(new BinaryOperatorExpression(BitWiseOpcodeNames[arg3], state.GetVar(arg2),
                                                                    state.GetVar(arg1))));
      break;
    ...
    case OP_LOADNULLS: {
      ...
      for (int i = 0; i < arg1; ++i) state.SetVar(arg0 + i, nullExpr);
    } break;
    ...
    case OP_GETVARGV:
      state.SetVar(arg0, ExpressionPtr(new ArrayIndexingExpression(
                             ExpressionPtr(new LiteralConstantExpression("vargv")), state.GetVar(arg1))));
      break;
    ...
    case OP_APPENDARRAY: {
      ...
      ExpressionPtr valueExp =
          (arg3 != 0) ? ExpressionPtr(new ConstantExpression(m_Literals[arg1])) : state.GetVar(arg1);
      ...
      if (arrayExp->GetType() == Exp_NewArrayExpression) {
        ...
        ExpressionPtr appendFunctionExp =
            ExpressionPtr(new ArrayIndexingExpression(arrayExp, ExpressionPtr(new ConstantExpression("append"))));
        shared_ptr<FunctionCallExpression> callExp =
            shared_ptr<FunctionCallExpression>(new FunctionCallExpression(appendFunctionExp));
    ...
    case OP_GETPARENT:
      state.SetVar(arg0, ExpressionPtr(new ArrayIndexingExpression(state.GetVar(arg1),
                                                                   ExpressionPtr(new ConstantExpression("parent")))));
      break;

    case OP_COMPARITH: {
      ExpressionPtr leftArg =
          ExpressionPtr(new ArrayIndexingExpression(state.GetVar(((unsigned int)arg1) >> 16), state.GetVar(arg2)));
      ExpressionPtr opExp =
          ExpressionPtr(new BinaryOperatorExpression((arg3 << 8) | '=', leftArg, state.GetVar(0x0000ffff & arg1)));
      state.SetVar(arg0, opExp, true);
    } break;

    case OP_COMPARITHL: {
      ExpressionPtr opExp =
          ExpressionPtr(new BinaryOperatorExpression((arg3 << 8) | '=', state.GetVar(arg1), state.GetVar(arg2)));
      state.SetVar(arg0, opExp, true);
    } break;
    ...
    case OP_CMP:
      state.SetVar(arg0, ExpressionPtr(new BinaryOperatorExpression(ComparisionOpcodeNames[arg3], state.GetVar(arg2),
                                                                    state.GetVar(arg1))));
      break;
    ...
    case OP_INSTANCEOF:
      state.SetVar(arg0, ExpressionPtr(new BinaryOperatorExpression(OperatorExpression::OPER_INSTANCEOF,
                                                                    state.GetVar(arg2), state.GetVar(arg1))));
      break;
    ...
    case OP_OR: {
      ...
      while (state.IP() < destIp && !state.EndOfInstructions()) {
        if (state.IP() == (destIp - 1) && m_Instructions[state.IP()].op == OP_MOVE &&
            static_cast<unsigned char>(m_Instructions[state.IP()].arg0) == arg0) {
          // Last move instruction of block - make simple move instead of variable set - that will be done later by
          // logic operator
          state.NextInstruction();
      ...
      ExpressionPtr opExpr =
          ExpressionPtr(new BinaryOperatorExpression((code == OP_OR) ? '||' : '&&', leftArg, state.GetVar(arg0)));
    ...
    case OP_CLOSURE: {
      shared_ptr<FunctionGeneratingExpression> func =
          shared_ptr<FunctionGeneratingExpression>(new FunctionGeneratingExpression(arg1, m_Functions[arg1]));

      for (vector<int>::const_iterator i = m_Functions[arg1].m_DefaultParams.begin();
           i != m_Functions[arg1].m_DefaultParams.end(); ++i)
    ...
    case OP_YIELD:
      state.PushStatement(StatementPtr(new YieldStatement((arg0 == 0xff) ? ExpressionPtr() : state.GetVar(arg1))));
      break;

    case OP_RESUME:
      state.SetVar(arg0,
                   ExpressionPtr(new UnaryOperatorExpression(OperatorExpression::OPER_RESUME, state.GetVar(arg1))));
      break;
    ...
    case OP_FOREACH: {
      ...
      //{
      //	const shared_ptr<LocalVarInitStatement> var =
      // static_pointer_cast<LocalVarInitStatement>(block->Statements().back());

      //	if (
      //		(refExp->GetType() == Exp_LocalVariable && var->GetStackAddress() == (arg2 + 2) &&
      // var->GetEndAddress() > state.IP() && var->GetEndAddress() < loopEndIp) || 		(valueExp->GetType() ==
      // Exp_LocalVariable && var->GetStackAddress() == (arg2 + 1) && var->GetEndAddress() > state.IP() &&
      // var->GetEndAddress() < loopEndIp) || 		(keyExp->GetType() == Exp_LocalVariable &&
      // var->GetStackAddress() == arg2
      //&& var->GetEndAddress() >= state.IP() && var->GetEndAddress() < loopEndIp)
      //	   )
      //	{
      ...
      while (!state.EndOfInstructions() && state.IP() < loopEndIp) {
        if (state.IP() == (loopEndIp - 1) && m_Instructions[state.IP()].op == OP_JMP &&
            m_Instructions[state.IP()].arg1 < 1) {
      ...
      LoopBaseStatementPtr stat =
          LoopBaseStatementPtr(new ForeachStatement(keyExp, valueExp, objectExp, state.PopBlock(block)));
    ...
    case OP_CLONE:
      state.SetVar(arg0,
                   ExpressionPtr(new UnaryOperatorExpression(OperatorExpression::OPER_CLONE, state.GetVar(arg1))));
      break;

    case OP_TYPEOF:
      state.SetVar(arg0,
                   ExpressionPtr(new UnaryOperatorExpression(OperatorExpression::OPER_TYPEOF, state.GetVar(arg1))));
      break;
    ...
    case OP_CLASS: {
      ...
      if (arg1 != -1) baseClass = state.GetVar(arg1);

      if (arg2 != 0xff) attributes = state.GetVar(arg2);
    ...
    case OP_NEWSLOT: {
      ...
      if (objExp->GetType() == Exp_NewTableExpression) {
        ...
        ExpressionPtr attributes;
        if (gotAttributes) attributes = state.GetVar(arg2 - 1);
        ...
        shared_ptr<ArrayIndexingExpression> derefExp =
            shared_ptr<ArrayIndexingExpression>(new ArrayIndexingExpression(objExp, keyExp));
    ...
    default:
      ...
      if (code != OP_JMP) {
        if (arg0 < m_StackSize) state.AtStack(arg0) = ExpressionPtr();
      }
...
void NutFunction::DecompileJumpZeroInstruction(VMState& state, int arg0, int jumpOffset) const {
  if (jumpOffset > 0 && (state.IP() + jumpOffset) < (int)m_Instructions.size() &&
      (state.IP() + jumpOffset) <= state.m_BlockState.blockEnd) {
    ...
    if (lastBlockOp.op == OP_JMP && lastBlockOp.arg1 < -jumpOffset) {
      ...
      if (state.m_BlockState.inLoop && state.m_BlockState.inLoop != BlockState::DoWhileLoop) blockLimit += 1;
      ...
      if ((state.IP() + jumpOffset + lastBlockOp.arg1) >= blockLimit) {
        ...
        while (state.IP() < destIp && !state.EndOfInstructions()) {
          if (state.IP() == (destIp - 1) && m_Instructions[state.IP()].op == OP_JMP &&
              m_Instructions[state.IP()].arg1 < 0) {
            // Skip loop jump
            state.NextInstruction();
          } else {
            DecompileStatement(state);
  ...
  if (jumpOffset >= 0 && (state.IP() + jumpOffset) <= state.m_BlockState.blockEnd) {
    ...
    if ((jumpOffset > 0) && (m_Instructions[ifBlockEndIp - 1].op == OP_JMP) &&
        (m_Instructions[ifBlockEndIp - 1].arg1 >= 0)) {
    ...
    if (!elseMatched) gotElseBlock = false;
    ...
    if (gotElseBlock) {
      ...
      if (ifBlockEndIp > 2 && m_Instructions[ifBlockEndIp - 2].op != OP_JZ &&
          m_Instructions[elseBlockEndIp - 1].op != OP_JMP) {
        ...
        if ((target1 == target2) && (target1 < m_StackSize) && state.AtStack(target1) &&
            stackCopy->at(target1).expression) {
...
void NutFunction::DecompileJumpInstruction(VMState& state, int arg1) const {
  ...
  while (switchBlock && !switchBlock->inSwitch) switchBlock = switchBlock->parent;
...
void NutFunction::DecompileSwitchBlock(VMState& state) const {
  // Function is called after parsing OP_JZ instruction and detecting switch chain patter - previous instructin should
  // be JZ
  ...
  // switch block
  if (pos < (int)m_Instructions.size() && m_Instructions[pos].op == OP_JMP && m_Instructions[pos].arg1 == 0) pos += 1;
  ...
  for (;;) {
    ...
    if (condition->GetType() == Exp_Operator) {
      ...
      if (operatorExpression->GetOperatorType() == '==') {
        shared_ptr<BinaryOperatorExpression> comparisionOperator =
            static_pointer_cast<BinaryOperatorExpression>(condition);

        if (!switchVariable) switchVariable = comparisionOperator->GetArg1();
    ...
    while (!state.EndOfInstructions() && state.IP() < currentBlockEnd) {
      if (state.IP() == (currentBlockEnd - 1) && m_Instructions[state.IP()].op == OP_JMP &&
          m_Instructions[state.IP()].arg1 > 0 &&
          (state.IP() + m_Instructions[state.IP()].arg1 + 1 < outCaseBlock.blockEnd)) {
        // Last forward jump of block - element of switch chain
        if (m_Instructions[state.IP()].arg1 > 0) nextBlockStart = state.IP() + 1 + m_Instructions[state.IP()].arg1;
    ...
    if (nextBlockStart > state.IP() && m_Instructions[nextBlockStart - 1].op == OP_JZ &&
        m_Instructions[nextBlockStart - 1].arg1 >= 0) {
      // Parse expression till next case condition
      while (!state.EndOfInstructions() && state.IP() < (nextBlockStart - 1)) DecompileStatement(state);
  ...
  if (state.IP() < state.m_BlockState.blockEnd) {
    ...
    while (!state.EndOfInstructions() && state.IP() < state.m_BlockState.blockEnd) DecompileStatement(state);
  }
...
void NutFunction::PrintOpcode(std::ostream& out, size_t pos, const Instruction& op) const {
  ...
  switch (code) {
    ...
    default:
      ...
      // out << "  0x" << std::setw(8) << std::setfill('0') << std::setbase(16) << op.arg1 << std::setfill(' ') <<
      // std::setbase(10);
      out << std::setw(5) << (int)op.arg1;
      out << std::setw(5) << (int)op.arg2;
      out << std::setw(5) << (int)op.arg3;
...
// ***************************************************************************************************************
void NutFunction::GenerateFunctionSource(int n, std::ostream& out, const std::string& name,
                                         const std::vector<std::string>& defaults) const {
  if (name != "constructor") out << "function ";
  out << name << '(';

  int paramsCount = 0;

  for (size_t i = 0; i < m_Parameters.size(); ++i) {
    if (i == 0 && m_Parameters[i] == "this") continue;

    if (paramsCount > 0) out << ", ";
  ...
  if (!m_OuterValues.empty()) {
    ...
    for (size_t i = 1; i < m_OuterValues.size(); ++i) out << ", " << m_OuterValues[i].name.GetString();
    out << ")";
  }
...
void NutFunction::GenerateBodySource(int n, std::ostream& out) const {
  ...
  if (m_IsGenerator) out << indent(n) << "// Function is a generator." << std::endl;
  ...
  if (g_DebugMode) {
    ...
    for (vector<NutFunction::LocalVarInfo>::const_reverse_iterator i = m_Locals.rbegin(); i != m_Locals.rend(); ++i) {
      out << indent(n) << "//   -" << i->name << spaces(10 - i->name.size()) << " // pos=" << i->pos
          << "  start=" << i->start_op << "  end=" << i->end_op << (i->foreachLoopState ? " foreach state" : "")
          << std::endl;
    }
  ...
  // Decompiler loop
  while (!state.EndOfInstructions()) {
    if (m_Instructions[state.IP()].op == OP_RETURN && (state.IP() == m_Instructions.size() - 1) &&
        m_Instructions[state.IP()].arg0 == -1) {
